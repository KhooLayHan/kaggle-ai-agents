...
def run_agent_cli(ticker: str):
    ...
    try:
        ...
        with Status(
            f"Analyzing {clean_ticker} using ADK Workflow...", spinner="dots"
        ) as status:
            for event in events:
                ...
                if event.author and event.author != current_agent:
                    ...
                    if current_agent and agent_buffer.strip():
                        ...
                        color = (
                            "yellow"
                            if current_agent == "MarketAnalyst"
                            else "magenta"
                ...
                if event.error_message:
                    ...
                    if "API key" in event.error_message:
                        console.print(
                            "[yellow]Hint: Please set the GEMINI_API_KEY environment variable.[/yellow]\n"
                        )
                    sys.exit(1)
            ...
            if current_agent and agent_buffer.strip():
                ...
                console.print(
                    Panel(
                        final_text, title=f"{current_agent} Output", border_style=color
                    )
                )
